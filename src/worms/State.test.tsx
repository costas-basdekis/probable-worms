...
describe("UnrolledState", () => {
  describe("getNextRolledStates", () => {
    it("gets no next states for state with 0 remaining dice", () => {
      expect(UnrolledState.fromDice([], 0).getNextRolledStates()).toEqual([]);
    });
    it("gets one state for each die side for state with 1 remaining die", () => {
      expect(UnrolledState.fromDice([], 1).getNextRolledStates()).toEqual([
    ...
    it("gets one state for each die side that wasn't picked already for state with 1-wide chest and 1 remaining die", () => {
      expect(UnrolledState.fromDice([1], 1).getNextRolledStates()).toEqual([
    ...
    it("gets one state for each unique 2-dice combination for state with 2 remaining dice", () => {
      expect(UnrolledState.fromDice([], 2).getNextRolledStates()).toEqual(
        ...
        .map(({diceRoll, count}) => ({
          rolledState: UnrolledState.fromDice([], 2).withRoll(diceRoll),
          count,
        }))
      );
    });
